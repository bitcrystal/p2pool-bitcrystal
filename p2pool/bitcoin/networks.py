...
nets = dict(
    ...
    quarkcoin=math.Object(
        P2P_PREFIX='fea503da'.decode('hex'),
        P2P_PORT=9299,
        ADDRESS_VERSION=0,
        RPC_PORT=19332,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
            'bitcrystaladdress' in (yield bitcoind.rpc_help()) and
            not (yield bitcoind.rpc_getinfo())['testnet']
        )),
        SUBSIDY_FUNC=lambda height: 9766*100000000 >> (height/1440)//60480,
        ...
        BLOCK_PERIOD=60, # s
        SYMBOL='BTCRY',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'BitCrystal_V20') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/bitcrystal_v20/') if platform.system() == 'Darwin' else os.path.expanduser('~/.bitcrystal_v20'), 'bitcrystal.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://bitcrystal.de:2750/',
        ADDRESS_EXPLORER_URL_PREFIX='http://bitcrystal.de:2750/address/',
